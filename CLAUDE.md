...
| `author_extraction` | Haiku | C13 strategy_author |
...
- **No reinventing** — Before implementing any utility (retry, logging, caching, singleton), search the existing codebase first. If similar logic exists in `A_core/` or any module, reuse it. Never reimplement.
- **f-strings only** — Use f-strings for all string formatting. Never use `%` formatting or `.format()`.
- **No hallucinated APIs** — When using external libraries (vLLM, scispacy, anthropic, unstructured), always verify the method signature exists. If unsure, read the import and check the source, don't guess.
- **Tests must assert real behavior** — Every test must have at least one meaningful assertion on actual output data. Tests that only check `is not None` or `len() > 0` are invalid. Never skip or weaken a test to make it pass — fix the code instead.
- **No test cheating** — If a test fails, fix the implementation, not the test. Never comment out assertions, reduce expected values, or add `pytest.skip()` to avoid a failure.
...
After every evaluation run that changes accuracy metrics, update `docs/DASHBOARD.md` with the latest results. This is the single source of truth for pipeline accuracy across all entity types and benchmarks.

## Documentation

Comprehensive docs in `docs/` folder:
- `docs/DASHBOARD.md` — **Accuracy dashboard** (must be updated after every eval change)
