...
SPLITS_TO_EVALUATE = [
    "dev"
]  # Which splits to run: ["dev"], ["test"], ["train"], or all
...
EVAL_ABBREV_CLASSES = [
    "disease"
]  # Only evaluate these abbreviation classes (disease, gene, identifier, general_medical, other)
...
def classify_abbreviation(
    short_form: str, long_form: Optional[str], lexicon_source: Optional[str] = None
) -> AbbreviationClass:
...
class AggregateMetrics:
    ...
    def precision(self) -> float:
        return (
            self.total_tp / (self.total_tp + self.total_fp)
    ...
    def recall(self) -> float:
        return (
            self.total_tp / (self.total_tp + self.total_fn)
...
def load_gold_standard(
    gold_path: Path, categories: Optional[List[str]] = None
) -> Dict[str, List[GoldEntry]]:
    ...
    if filtered_count > 0:
        print(
            f" Filtered out {filtered_count} entries (categories not in {categories})"
        )
...
def lf_matches(
    sys_lf: Optional[str], gold_lf: Optional[str], threshold: float = 0.8
) -> bool:
...
def compare_extractions(
    ...
    if eval_classes:
        filtered_extracted = [
            ext for ext in extracted if ext.abbrev_class.value in eval_classes
        ]
        filtered_count = len(extracted) - len(filtered_extracted)
        if filtered_count > 0:
            print(
                f"      Classification filter: {len(extracted)} total -> {len(filtered_extracted)} disease-class (filtered {filtered_count})"
            )
...
def evaluate_split(
    ...
    for i, pdf_path in enumerate(pdf_with_gold, 1):
        ...
        try:
            ...
            metrics, class_counts = compare_extractions(
                extracted, gold_entries, eval_classes=EVAL_ABBREV_CLASSES
            )
            ...
            print(
                f"      Results:          TP={metrics.tp}, FP={metrics.fp}, FN={metrics.fn}"
            )
            print(
                f"      Metrics:          P={metrics.precision:.1%}, R={metrics.recall:.1%}, F1={metrics.f1:.1%}"
            )
...
def main():
    ...
    print(
        f" Loaded {sum(len(v) for v in gold_by_doc.values())} annotations across {len(gold_by_doc)} documents"
    )
