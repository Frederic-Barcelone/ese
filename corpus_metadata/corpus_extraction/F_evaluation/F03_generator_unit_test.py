#!/usr/bin/env python3
# corpus_metadata/corpus_extraction/F_evaluation/F03_generator_unit_test.py
"""
Generator Unit Test
...
def extract_abbreviations_simple(text: str) -> List[Tuple[str, str]]:
    ...
    pattern_a = re.compile(
        rf"([A-Za-zÀ-ÿ][{LF_CHARS}]{{3,120}})\s*\(([A-Z0-9][{SF_CHARS}]{{1,40}})\)",
        re.IGNORECASE,
    )
    ...
    pattern_b = re.compile(
        rf"\b([A-Z0-9][{SF_CHARS}]{{1,40}})\s*\(([A-Za-zÀ-ÿ][{LF_CHARS}]{{3,120}})\)",
        re.IGNORECASE,
    )
    ...
    pattern_c = re.compile(
        rf"\b([A-Z0-9][{SF_CHARS}]{{1,40}})\s*,\s*defined as\s+([A-Za-zÀ-ÿ][{LF_CHARS}]{{3,120}})",
        re.IGNORECASE,
    )
